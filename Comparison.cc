...
char* OrderMaker::toString() {
	...
	//cout<<toReturn<<endl<<"oh yeah";
	//cout<<endl<<"which type value "<<whichTypes[0]<<endl ;
	for (int i = 0; i < numAtts; i++) {
		char t[10];
		sprintf(t, "%d ", whichAtts[i]);
		toReturn.append(t);
	}
	toReturn += "\n";
	for (int i = 0; i < numAtts; i++) {
		char t[10];
		sprintf(t, "%d ", whichTypes[i]);
		toReturn.append(t);
	}
	//toReturn+="\n";
...
void OrderMaker::setAtrr(int numAtts, int which[], int types[]) {
...
int CNF::createQuery(OrderMaker& sortOrder, OrderMaker& output) {
	output.numAtts = 0;
	OrderMaker temp;
	if (!GetSortOrders(temp))
		return 0;
	...
	for (int i = 0; i < sortOrder.numAtts; i++) {
		...
		for (int j = 0; j < temp.numAtts; j++) {
			...
			if (sortOrder.whichAtts[i] == temp.whichAtts[j]
					&& sortOrder.whichTypes[i] == temp.whichTypes[j]) {
				output.whichAtts[i] = sortOrder.whichAtts[i];
				output.whichTypes[i] = sortOrder.whichTypes[i];
				flag = false;
				output.numAtts++;
			}
		}

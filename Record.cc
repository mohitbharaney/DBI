...
void Record :: WriteToFile (FILE *opFile,Schema *mySchema) {
...
// check syntax for \n in file and if not working remoce %s
//cout << "\n";
fprintf(opFile,"\n");
}

int Record :: ComposeNewRecord (Schema *mySchema, const char *src)
{
char *space = new (std::nothrow) char[PAGE_SIZE];
if(space==NULL)
{
cout << "ERROR : Memory is not enough. EXIT !!!\n";
exit(1);
}
char *rSpace=new (std::nothrow) char[PAGE_SIZE];
if(rSpace==NULL)
{
cout << "ERROR : Memory is not enough. EXIT !!!\n";
exit(1);
...
int NoOfAttri=mySchema->GetNumAtts();
Attribute *attribute=mySchema->GetAtts();

int currentPos=sizeof(int) *(NoOfAttri+1); //this is the current position (int bytes) in the binary

// loop through all the attributeor
int cursor=0;

for(int i=0;i<NoOfAttri;i++)
{
int length=0;
while(1)
{
...
{
*((int *) &(rSpace[currentPos])) = atoi (space);	
...
// make sure that we are starting at a double-aligned position;
// if not, then we put some extra space in there
while (currentPos % sizeof(double) != 0) {
currentPos += sizeof (int);
((int *) rSpace)[i + 1] = currentPos;
}

*((double *) &(rSpace[currentPos])) = atof (space);
...
strcpy (&(rSpace[currentPos]), space); 
currentPos += length;
...
// the last thing is to set up the pointer to just past the end of the rec0rd
((int *) rSpace)[0] = currentPos; 

// and copy over the bits
bits = new (std::nothrow) char[currentPos];
if (bits == NULL)
{
cout << "ERROR : memory is not enough. EXIT !!!\n";
exit(1);
